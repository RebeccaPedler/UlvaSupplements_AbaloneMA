...
## Count number of unique study_ID (observations) per outcome category
mydata %>%
...
## Create bubble plot of species vs outcome
...
bubble_data <- bubble_data %>%
  mutate(
    species_label = gsub(" x ", "\n x \n", species),
    species_label = gsub(" and ", "\n and \n", species_label)
  )

# Build bubble plot
bubble_plot_species <- ggplot(
  bubble_data,
  aes(
    x = species,
    y = fct_rev(fct_infreq(outcome_long))
  )
) +
  geom_point(
...
# Plot
journal_year <- ggplot(
  bubble_data_publication,
  aes(
    y = journal,
    x = as.factor(publication_year)
  )
  ) +
  geom_point(
    aes(
      size = unique_study_count,
      colour = publication_type
    ),
    alpha = 0.7
    ) +
    scale_colour_manual(
    values = c(
      "Primary literature"   = "steelblue",
      "Grey literature" = "grey60"
    )
  ) +
  scale_size_continuous(range = c(3, 15)) +
  guides(size = "none") + 
  scale_x_discrete(limits = as.character(2011:2025)) +
  labs(
    y = "Journal",
    x = "Publication year",
...
node_levels <- c(
  ...
  # Study conditions
  sort(unique(mydata_alluvial$study_conditions)),
  # Preparation
  sort(unique(mydata_alluvial$intervention_preparation)),
  # Species (leave as-is)
  sort(unique(mydata_alluvial$species)),
  # Dose (descending order)
  ">30%", "20-30%", "10-20%", "5–10%", "2.5–5%", "≤2.5%",
  # Duration (descending order)
  ">180 d", "120–180 d", "60–120 d", "30-60 d", "≤30 d",
  # Size (descending order)
  ">50 g", "10–50 g", "1–10 g", "≤1 g"
)
...
sankey_plot <- ggplot(
  ...
  sankey_p_label(size = 4, color = "white", fill = "gray10", alpha = 0.6) +
...
write_csv(mydata, "cleaned_data_for_meta_analysis.csv")

### End of script ###

### NEXT ###: Use cleaned data from the output file *"cleaned_data_for_meta_analysis.csv"* to calculate effect sizes (lnRR). 
